...
export function buildParseQueryPrompt(query: string, context: UserContext) {
  ...
  const systemPrompt = [
    ...
    "",
    "  1. LOCATION. If the query contains a city or country word that appears",
    "     in the user's cities/countries list below, set `hard.city` (and",
    "     `hard.country` when unambiguous). This step is ATOMIC and INDEPENDENT",
    "     of every other check. It runs even if the same word also appears in",
    "     a tag/list/sub-cat name.",
    "",
    ...
    "  - city — IF the query mentions a city word that matches the user's",
    "    cities list. THIS IS NON-OPTIONAL. Examples: 'in London', 'Barcelona",
    "    cafes', 'restaurants Istanbul'. Set `hard.city`. Setting any tag/list/",
    "    sub-cat boost with the same word does NOT release this requirement.",
    "  - country — same rule. Most users say city, but 'in the UK' → country.",
    ...
    "    (user has city 'London', tag 'london', tag 'Date Spot', list 'london trip')",
    "  hard: { category_ids=[restaurant_id], city='London' }    ← city SET",
    ...
    "  WHY: 'london' triggers BOTH hard.city (step 1, atomic) AND tag/list",
    "       boosts (step 6). They coexist. 'date' is semantic → soft + boost.",
    "",
    ...
    "          subcategory_ids=[fine_dining_id] }              ← all three SET",
    "  soft_features: {}",
    "  boosts: { matching_tag_ids=[london_tag_id],",
    "           matching_list_ids=[london_trip_id] }",
    "  semantic_intent: 'fine dining restaurants in London'",
    "  requires_semantic_ranking: false",
    "  WHY: 'fine dining' literally names a sub-cat → hard. 'london' is in",
    "       cities → hard.city. SAME word matches tag+list names → also boost.",
    "       Hard and boost coexist; never replace one with the other.",
    "",
    ...
    "    (user has city 'Barcelona', no tag/list named like it)",
    "  hard: { category_ids=[cafe_id], city='Barcelona' }",
    "  soft_features: {}",
    "  boosts: {}",
    "  semantic_intent: 'Barcelona cafes, good quality'",
    "  requires_semantic_ranking: true",
    "  WHY: 'barcelona' is in cities → hard.city. 'good' → rerank trigger.",
    "       No curated taxonomy match, so boosts empty.",
    "",
    "Example 7 — 'places in istanbul I haven't been to'",
    "    (user has city 'İstanbul' with Turkish diacritic)",
    "  hard: { city='İstanbul', visit_status='want_to_go' }",
    "  soft_features: {}",
    "  boosts: {}",
    "  requires_semantic_ranking: false",
    "  WHY: Match the user's canonical city spelling, even if the query used",
    "       a transliteration. 'haven't been to' → visit_status filter.",
    "",
    ...
    "    (city missing — query mentions London but it's not in hard)",
    "  ✓ RIGHT: hard={ category_ids, city='London', subcategory_ids },",
    ...
    "  cities list, ALWAYS set hard.city. This rule has no exceptions.",
