...
/**
 * v1.21.0 (S3 AI-02): the GET /api/stats aggregation, extracted verbatim
 * so the route and the assistant's `get_stats` tool share ONE
 * implementation. Response shape matches StatsData (use-stats.ts).
 */
export async function computeUserStats(
  ...
  let visitStatus: Record<string, number> = {
    want_to_go: 0,
    booked: 0,
    visited: 0,
    favorite: 0,
  ...
  const byCategory = Array.from(catCounts.values()).sort(
    (a, b) => b.count - a.count
  );
  ...
  // Rating distribution
  const ratingDist: { rating: number; count: number }[] = [1, 2, 3, 4, 5].map(
    (r) => ({ rating: r, count: 0 })
  );
