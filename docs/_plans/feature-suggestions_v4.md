...
- ✅ **S4 (Trip Intelligence) kapandı** (v1.22.0, 16.07 merge): NF-07
  rota modu (routing_profile + toggle + mapbox_directions SKU), NF-08
  bütçe (cost_estimate/currency/party_size + backfill), AI-09 AI Plan
  (idx şema, delete-after-validate, gün narrative'i). İki hotfix:
  Gemini repetition-loop → thinking KAPALI + şemasız JSON kipi
  (structuredOutputs:false + thinkingBudget:0, gemini.md'ye ders yazıldı).
- ✅ **v1.22.1**: Mapbox-search önizlemesi ↔ yapıştır akışı lite-profile
  paritesi (kullanıcı mobilde yakaladı; buildLiteProfileForResponse
  paylaşılan lib'e çıktı) + iki formda kategori-değişince-subcategory
  temizliği.
- 🟡 **v1.23.0 (asistan↔AI-search paritesi) PR'da**: rank_places tool'u
  (rank-results'ın server-side ikizi, tag-boost garantili koltuklar),
  panelde "Show all on map/as list" push'u (ai-search-store köprüsü —
  banner/kaydetme/why bedava), soft-sorgu yönlendirmesi. Karar: mobil
  AI-search bar EKLENMEYECEK — asistan dolduracak; AI-01 bar'ı emeklilik
  kriterleri sağlanana kadar dokunulmadan yaşıyor.
...
> **v1.23.0 birleşme adımı (16.07.2026):** Asistan, AI-01'in yaptığını
> yapabilir hale getirildi — `rank_places` tool'u (LLM-as-judge'ın
> server-side ikizi; agent yalnız soft/öznel kriterde çağırır;
> `boost_tag_ids` ile taksonomiden semantik tag'ler aday havuzuna
> garantili koltuk alır) + sonuç kartları altında "Show all on map /
> as list" push'u (aynı ai-search-store: banner, temizleme, why-satırları,
> SaveFilterButton değişmeden çalışır). **AI-01 bar'ına dokunulmadı;
> mobile bar eklenmeyecek (asistan dolduruyor). Emeklilik kriterleri:**
> (1) yan-yana altın-sorgu setinde asistan+push ≈ AI search kalitesi,
> (2) CostTracker'da ai_parse_query kullanımının doğal düşüşü (birkaç
> hafta gözlem), (3) kullanıcı onayı.
> **Gerçekleşen (16.07.2026, kullanıcı kararı):** görünürlük emekliliği
> kademeli kriterleri beklemeden uygulandı — bar hiçbir yerde render
> edilmiyor (`AiSearchInput bannerOnly`); endpoint'ler/pipeline duruyor
> (kayıtlı ✨ chip'ler + asistan rank/push kullanıyor). Aktif-durum
> banner'ı (sorgu + ✕ + broaden) korundu.
